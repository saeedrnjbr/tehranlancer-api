...
@section('content')
    ...
    @component('components.dashboard-layout', [
        ...
        'link' => route('admin.course_categories.create'),
    ])
        <div
                        ...
                        @if ($rows->isNotEmpty())
                            @foreach ($rows as $row)
                                <tr>
                                    ...
                                    <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                        <p class="text-sm">{{ $row->parent ? $row->parent->name : "ندارد" }}</p>
                                    </td>
                                    ...
                                    <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        @include('partials.operation', [
                                            'removeLink' => route('admin.course_categories.remove', ['id' => $row->id]),
                                            'editLink' => route('admin.course_categories.show', ['id' => $row->id]),
                                        ])
                                    </td>
                                </tr>
                            @endforeach
