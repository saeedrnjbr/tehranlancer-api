                        ...
                        @if ($rows->isNotEmpty())
                            @foreach ($rows as $row)
                                <tr>
                                    ...
                                    <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                        <p class="text-sm">{{ $row->product_category->name }}</p>
                                    </td>
                                    <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                        <p class="text-sm">{{ number_format($row->price) }} تومان</p>
                                    </td>
                                    ...
                                    <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        @include('partials.operation', [
                                            'removeLink' => route('admin.products.remove', ['id' => $row->id]),
                                            'editLink' => route('admin.products.show', ['id' => $row->id]),
                                        ])
                                    </td>
                                </tr>
                            @endforeach
