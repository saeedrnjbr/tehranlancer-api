...
@section('content')
    ...
    @component('components.dashboard-layout', [
        ...
        'link' => route('admin.lessons.create'),
    ])
        <div
                        ...
                        @if ($rows->isNotEmpty())
                            @foreach ($rows as $row)
                                <tr>
                                    ...
                                    <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                        <p class="text-sm">{{ $row->duration ?? '' }}</p>
                                    </td>
                                    <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                        <p class="text-sm">{{ $row->course->name ?? '' }}</p>
                                    </td>
                                    ...
                                    <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        @include('partials.operation', [
                                            'removeLink' => route('admin.lessons.remove', ['id' => $row->id]),
                                            'editLink' => route('admin.lessons.show', ['id' => $row->id]),
                                        ])
                                    </td>
                                </tr>
                            @endforeach
