    ...
    @component('components.dashboard-layout', [
        ...
        <div class="flex flex-col gap-9">
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <form enctype="multipart/form-data" method="post" action="{{ !empty($edit) && $edit->id ? route("admin.events.update", ["id" => $edit->id]) : route("admin.events.store") }}" class="flex flex-col gap-5.5 p-6.5">
                    ...
                    <div>
                        ...
                        <input value="{{ !empty($edit) && $edit->expired_at ? $edit->expired_at_fa : @old("expired_at") }}"  type="text" name="expired_at" class="w-full persianDatepicker rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                    </div>
                    <div>
                        <label class="mb-3 block text-sm font-medium text-black dark:text-white">
                            دسته‌بندی رویداد 
                        ...
                        >
                            @foreach ($event_categories as $event_category)
                                <option @if(!empty($edit) && $edit->event_category_id == $event_category->id) selected @endif value="{{  $event_category->id }}">{{  $event_category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <div class="flex gap-5 items-center justify-between">
                            ...
                            @if (!empty($edit->image))
                                <div class="relative">
                                    ...
                                    <a href="{{ route("admin.events.remove.image", ["id" => $edit->id ]) }}">
