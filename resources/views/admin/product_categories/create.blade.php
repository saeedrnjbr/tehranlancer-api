...
@section('content')
    @component('components.dashboard-layout', [
        'title' => __('pages.product_categories'),
    ])
        <div class="flex flex-col gap-9">
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
               
                <form enctype="multipart/form-data" method="post" action="{{ !empty($edit) && $edit->id ? route("admin.product_categories.update", ["id" => $edit->id]) : route("admin.product_categories.store") }}" class="flex flex-col gap-5.5 p-6.5">
