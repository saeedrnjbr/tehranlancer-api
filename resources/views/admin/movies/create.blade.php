        ...
        <div class="flex flex-col gap-9">
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <form enctype="multipart/form-data" method="post"
                    action="{{ !empty($edit) && $edit->id ? route('admin.movies.update', ['id' => $edit->id]) : route('admin.movies.store') }}"
                    ...
                    <div>
                        ...
                        <input value="{{ !empty($edit) && $edit->age_group ? $edit->age_group : @old('age_group') }}"
                            type="text" name="age_group"
                            class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                    </div>
                    ...
                    <div>
                        ...
                        <input value="{{ !empty($edit) && $edit->link ? $edit->link : @old('link') }}" type="text"
                            name="link"
                            class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            value="{{ !empty($edit) && $edit->thumbnail_link ? $edit->thumbnail_link : @old('thumbnail_link') }}"
                            type="text" name="thumbnail_link"
                            class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="genre_id"
                            ...
                            @foreach ($genres as $genre)
                                <option @if (!empty($edit) && $edit->genre_id == $genre->id) selected @endif value="{{ $genre->id }}">
                                    {{ $genre->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="is_offer"
                            class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-12 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                            <option value="">انتخاب کنید</option>
                            <option @if (!empty($edit) && $edit->is_offer == '1') selected @endif value="1">فعال</option>
                            <option @if (!empty($edit) && $edit->is_offer == '0') selected @endif value="0">غیرفعال</option>
                        </select>
                    </div>
                    <div>
